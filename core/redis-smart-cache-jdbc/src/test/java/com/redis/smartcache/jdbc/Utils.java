...
public class Utils {
    ...
    public static void assertEquals(ResultSet expected, ResultSet actual) throws SQLException {
        ...
        while (expected.next()) {
            ...
            for (int columnIndex = 1; columnIndex <= expectedMetaData.getColumnCount(); columnIndex++) {
                String columnInfo = String.format("Column %s type %s (%s vs %s)", expectedMetaData.getColumnName(columnIndex),
                        expectedMetaData.getColumnTypeName(columnIndex), expectedMetaData.getColumnClassName(columnIndex),
    ...
    public static void assertEquals(ResultSetMetaData expectedMetaData, ResultSetMetaData actualMetaData) throws SQLException {
        ...
        for (int index = 1; index <= expectedMetaData.getColumnCount(); index++) {
            ...
            Assertions.assertEquals(expectedMetaData.getColumnDisplaySize(index), actualMetaData.getColumnDisplaySize(index));
        }
    }
